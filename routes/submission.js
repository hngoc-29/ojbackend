...
router.post('/:id/run', async (req, res) => {
    ...
    try {
        ...
        // Chỉ chạy khi status là "not_run"
        ...
        // Thông báo client đã bắt đầu chạy
        ...
        // Update trạng thái và reset testStatuses
        ...
        for (let i = 0; i < total; i++) {
            ...
            if (compile.status !== 0) {
                ...
                statuses.push('compile_error');
                await Submission.findByIdAndUpdate(id, {
                    status: 'compile_error', msg: errMsg, score: 0, testStatuses: statuses
                });
                io.emit(`submission_${id}`, { index: i, status: 'compile_error', message: errMsg });
                io.emit(`submission_${id}`, { done: true, score: 0, status: 'compile_error' });
                return;
            }
